...
def tag(product, version, options) :
    try:
        ...
        redmine_url = 'http://' + options.repository + '/redmine'
        home = expanduser("~")
        redmine_key = open(home + '/.redmine_apikey', 'r').read().replace('\n', '')

        redmine = Redmine(redmine_url, key=redmine_key)
        project = get_project(redmine, options.redmineid if options.redmineid != None else product)
        ...
        replace(os.path.join(cwd, '%s' % (options.properties)), '%s.version=.*' % (product), '%s.version=%s' % (product, version))
        ...
        if options.noconfirm :
            ...
            while True :
                try:
                    result = strtobool(raw_input("Do you really want to push the tag to origin ? (y/n) "))
                except ValueError:
                    pass
                else:
                    break
        ...
        if result:
            ...
            execute(cmd, cwd=cwd)
            webbrowser.open('http://' + options.repository + '/redmine/projects/' + project.identifier + '/issues')
            print 'Done'
        else :
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(version='1.0',description='Tag a product version automatically')
    parser.add_argument('product', help='Product name')
    parser.add_argument('version', help='Product version')

    parser.add_argument('-r','--repository', help='Gitlab repositiory for the product', default='simforge.tls.fr.astrium.corp')
    parser.add_argument('-g','--group', help='Gitlab groupname for the product', default='simtg')
    parser.add_argument('-p','--properties', help='versions.properties path', default='ant/versions.properties')

    parser.add_argument('--noconfirm', help='Skip confirmation prompt before pushing to origin', action="store_true", default=False)

    parser.add_argument('--redmineid', help='Redmine product id if different from product name')
